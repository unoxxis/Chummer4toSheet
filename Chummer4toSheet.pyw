...
import chumdata
from chumchar import (ImportChummerCharacter, RecalculateCharacter,
                      SaveCharacter)
...
if __name__ == "__main__":
    ...
    ap = argparse.ArgumentParser(
        description="A program to convert Chummer4 XML Character files "
        "(*.chum) into PDF character sheets.",
    ...
    ap.add_argument(
        "-c",
        "--configfilename",
        default=_DEFAULT_CFGFILENAME_,
        help="Filename of the config file (default: %(default)s)",
    )
    ap.add_argument(
        "-l",
        "--logfilename",
        default=_DEFAULT_LOGFILENAME_,
    ...
    lh_logfile = logging.handlers.RotatingFileHandler(
        options.logfilename,
        mode="a",
        encoding="utf-8",
        maxBytes=1024 * 1024,
